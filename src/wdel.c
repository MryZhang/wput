...
#  endif
...
#ifndef WIN32
    /* If we are still at stdout, then redirect output to 'wdel-log'. */
    if(opt.background) {
        if(opt.output == stdout) {
                printout(vLESS, _("Resuming in background. Logging output to 'wdel-log'.\n"));
                opt.output   = fopen("wdel-log", "a");
    ...
    if(opt.transfered == 0 && opt.skipped == 0 && opt.failed == 0)
        printout(vNORMAL, _("Nothing done. Try `wdel --help'.\n"));
    else
    ...
    if(opt.failed > 0)
        printout(vNORMAL, opt.failed == 1 ? _("Deletion of %d file failed.\n") : _("Deletion of %d files failed.\n"), opt.failed);
...
int read_urls(void) {
    ...
    while ((url = p = read_line(opt.input))) {
        ...
        if(!strncmp(p, "ftp://", 6))
            wdel_queue_add_entry(NULL, cpy(p));
        else
            wdel_queue_add_file(cpy(p));
...
void commandlineoptions(int argc, char * argv[]){
    ...
    while (1)
    {
        c = getopt_long (argc, argv, "Y:Vhbo:a:dqvn:i:t:T:w:ps",
        ...
        case 0:
            ...
            case  6: //compile-options
                fprintf(opt.output, "wdel version: %s\n\n#defined options:\n", version);
        ...
        case 'V':
            fprintf(opt.output, _("wdel version: %s\n"), version);
            exit(0);
        case 'h':
        default:
            fprintf(stderr, _("Usage: wdel [options] [url [file]...]...\n"
"  url        ftp://[username[:password]@]hostname[:port][/[path/][file]]\n\n"
"Startup:\n"
"  -V, --version         Display the version of wdel and exit.\n"
...
"       --basename=PATH         snip PATH off each file when appending to a URL\n"
"\n"));
            fprintf(stderr, _(
"Connection:\n"
...
"       --force-tls             force the useage of TLS\n"));
            ...
            fprintf(stderr, _(
"\n"
"See wdel(1) for more detailed descriptions of the options.\n"
...
"http://sourceforge.net/tracker/?group_id=141519\n"));
    ...
    while(optind < argc) {
        if(!strncmp(argv[optind], "ftp://", 6))
            wdel_queue_add_entry(NULL, cpy(argv[optind]));
        else
            wdel_queue_add_file(cpy(argv[optind]));
        optind++;
    }
}
